function [h0,h1,h2,h3,h4,g0,g1,g2,g3,g4] = Design5BandRedundantHP(h0)

% 27-Sep-2011
% Design 5-band sibling frame with a given h0.
% The synthesis and analysis highpass filters are redundant. 
% The bandpass filters are modulated versions of one another.
%
% NOTE: filters normalize to 4, NOT 2

% Generate LP filter using symflat(Zpi,L):
% Zpi zeros at z=-1, 2L zeros away from z=-1

h0 = h0(:);
g0 = flip(h0);

%PR cond 2:
rh0 = conv(altsign(h0),g0);
rts = roots(rh0);
...
rtsh1 = input('Choose zeros of H1(z) (A(z)??):');
h1 = poly(rtsh1);
h1 = real(h1);
%h1 = conv(h1,flip(h1));
h1 = h1(:);

% for nonsymmetric case do h2 = makeh1(h1)...
h2 = altsign(h1);
...
%% Examples for tight nonsymmetric case:

H = [ -0.0625    0.0118    0.1649    0.0176
         0    0.0237   -0.3299    0.0859
    0.5625   -0.1649   -0.0118   -0.2004
    1.0000   -0.3536    0.3536   -0.7930
    0.5625   -0.0118   -0.1649    0.8900
         0    0.3299   -0.0237         0
   -0.0625    0.1649    0.0118         0]



G = [   -0.0625   -0.1649   -0.0118         0
         0   -0.3299    0.0237         0
    0.5625    0.0118    0.1649    0.8900
    1.0000    0.3536   -0.3536   -0.7930
    0.5625    0.1649    0.0118   -0.2004
         0   -0.0237    0.3299    0.0859
   -0.0625   -0.0118   -0.1649    0.0176]
